	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo, sem viadagens 😡*${mdata.subject}*\n ePor favor não seja um ghost❤️`
				...
				teks = `Tchau @${num.split('@')[0]} mó troxa 😔 abandonou o bonde`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				error: {
					stick: '⚠️ Falha, tenta dnv manin ❤️ ocorreu um erro ao converter a imagem em figurinha ⚠️',
					Iv: '❌ Link tidak valid ❌'
				},
				only: {
			...
			const ownerNumber = ["5521982660837@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg.result, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							fs.writeFileSync(ranp, buffer, (err) => {
								if (err) return reply('Falha, ocorreu um erro, tenta dnv aí, meu bom 😽❤️.')
							})
				...
				case 'porno':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Adm corno proibiu porno no gp🙄'})
					break
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/d1M6lOz.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* DOKI PLAYER 13\n*YOUTUBE:* https://bit.ly/30Iowvl\n*WPP:* wa.me/+5521982660837\n*INSTA:* https://www.instagram.com/dokitigronso/?hl=pt-br\n\n\n*Digite *.bot* para ver comandos basicos para criar um bot'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'loli1':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'hmm lolicon, então quer ver lolis?'})
					break
				case 'hentai':
					memein = await kagApi.memeindo()
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do doki:*\n\n https://bit.ly/30Iowvl'})
					break
				case 'mia1':
					memein = await kagApi.memeindo()
